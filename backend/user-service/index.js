...
const users = [
  {
    ...
    documents: { license: null, criminalRecord: null },
  },
  {
    id: 2,
...
app.post(
  ...
  (req, res) => {
    ...
    if (!req.files?.license) {
      return res
        .status(400)
        .json({ error: "Driver's license document is required." });
    }
    if (!req.files?.criminalRecord) {
      return res
        .status(400)
        .json({ error: "Criminal record document is required." });
    }
    ...
    console.log(
      `[UserService] ✓ Register → ${newUser.name} (${roleLabel(newUser.role)}) | License: ${req.files.license[0].filename} | Criminal: ${req.files.criminalRecord[0].filename}`,
    );
...
app.post("/api/login", (req, res) => {
  ...
  console.log(
    `[UserService] ✓ Login  → ${user.name} (${roleLabel(user.role)}) | Active sessions: ${Object.keys(tokens).length}`,
  );
...
app.post("/api/logout", (req, res) => {
  ...
  if (auth) {
    ...
    if (user) {
      delete tokens[token];
      console.log(
        `[UserService] ✓ Logout → ${user.name} | Active sessions: ${Object.keys(tokens).length}`,
      );
    }
  }
...
if (require.main === module) {
  app.listen(3001, () => {
    ...
    console.log(
      `   ${users.length} users: ${owners} car owner(s), ${renters} renter(s)`,
    );
  });
}
